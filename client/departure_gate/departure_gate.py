...
# Global flag used to avoid duplicate departure messages
waiting_for_confirmation = False

# Initialize WS2812 LED strip (for visual feedback)
NUM_PIXELS = 8
...
def process_message(client, userdata, message):
    ...
    try:
        ...
        # Optionally, ignore messages sent by ourselves
...
def response_controller(received_dict: dict):
    ...
    For departure, we expect either a confirmation response or a parking status update.
    """
    ...
    if header == 'confirmed':
        ...
        print("Unhandled message header:", header)

def register_departure(card_uuid: str):
    """
    Sends an MQTT message registering the departure.
    The card_uuid is provided by the RFID reader.
    """
    ...
    print("Registering departure with card UUID:", card_uuid)
    # Note: The header is now 'departure' to distinguish from the entry gate.
    message_string = messenger.build_message("departure", database_message)
    print("Sending message:", message_string)
...
def light_green_led():
    """Light the green LED for 3 seconds."""
    pixels.fill((0, 255, 0))
    pixels.show()
    time.sleep(3)
    pixels.fill((0, 0, 0))
...
def update_parking_status(received_dict: dict):
    """Update the OLED display with the current parking status."""
...
def poll_rfid():
    """
    Continuously polls for an RFID card at the departure gate.
    When a card is detected, its UID is formatted and used to register the departure.
    """
    MIFAREReader = MFRC522()
    print("Place your card near the RFID reader for departure...")
    
    while True:
        # Request a card (idle state)
        (status, TagType) = MIFAREReader.MFRC522_Request(MIFAREReader.PICC_REQIDL)
        if status == MIFAREReader.MI_OK:
            # Card found; read its UID.
            (status, uid) = MIFAREReader.MFRC522_Anticoll()
            if status == MIFAREReader.MI_OK:
                # Format UID as a hexadecimal string.
                card_uuid = "".join([f"{byte:02X}" for byte in uid])
                print("RFID card detected:", card_uuid)
                register_departure(card_uuid)
                # Wait a few seconds to avoid multiple registrations for the same card.
                time.sleep(3)
        time.sleep(0.1)

if __name__ == '__main__':
    print("Starting Departure Gate with RFID")
    connect()
